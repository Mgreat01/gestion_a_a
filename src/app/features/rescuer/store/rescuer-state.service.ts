...
export class RescuerStateService {
  ...
  loadNearby(latitude: number, longitude: number): void { this.alertsApi.nearby(latitude, longitude).pipe(catchError(error => { this.fail(error); return EMPTY; })).subscribe(alerts => this.nearbyAlerts.set(this.unique(alerts))); }
  ...
  private handleMessage(message: WebSocketAlertMessage): void {
    ...
    if (['alert_assigned', 'alert_updated', 'alert_resolved'].includes(message.type) && this.isAlert(data)) this.upsert(data);
  }
